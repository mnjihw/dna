...
void __fastcall wzcrypt_decrypt(unsigned int size, unsigned int iv, unsigned char *in, unsigned char *out)
{
	static unsigned char IV[4];
	unsigned int remaining = size;
	static unsigned int length, idx;
	static unsigned int start = 0;
	static unsigned char realIV[16];
	static aes256_context *context = &ctx;


	*(unsigned int*) &IV[0] = iv; //이건 대략 맞는듯
	length = 0x5B0;
	start = 0;
	...
	while (remaining > 0)
	{
		...
		{
			if (((idx - start) % sizeof(realIV)) == 0)
				aes256_encrypt_ecb(context, realIV);
