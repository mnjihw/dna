...
cque_t* cque_alloc(unsigned int size)
{
	...
	VIRTUALIZER1_START
	{
		cque = (void *) HeapAlloc((HANDLE) GetProcessHeap(), 0, (size > sizeof(((cque_t *) 0)->data)) ? ((sizeof(cque_t) - sizeof(((cque_t *) 0)->data)) + size) : sizeof(cque_t));
...
#endif
		HeapFree((HANDLE) GetProcessHeap(), 0, cque);
	}
		VIRTUALIZER_END
}
