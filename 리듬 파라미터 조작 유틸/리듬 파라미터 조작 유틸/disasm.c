...
static void DecodeRG(int index, int datasize, int type) {
	...
	if (mode >= DISASM_FILE) {
		...
		if (type < PSEUDOOP)                 // Not a pseudooperand
			nresult += wsprintf(da->result + nresult, "%s", name);
		;
	};
};
...
static void DecodeST(int index, int pseudoop) {
	...
	index &= 0x07;
	i = wsprintf(s, "%s(%i)", (lowercase ? "st" : "ST"), index);
	if (pseudoop == 0) {
...
static void DecodeMX(int index) {
	...
	nresult += wsprintf(pr, "%s%i", (lowercase ? "mm" : "MM"), index);
};
...
static void Memadr(int defseg, const char *descr, long offset, int dsize) {
	...
	if (showmemsize != 0 || (da->cmdtype & C_TYPEMASK) == C_MMX ||
		...
		if (dsize < sizeof(sizename) / sizeof(sizename[0]))
			n += wsprintf(pr + n, "%s %s", sizename[dsize], (ideal == 0 ? "PTR " : ""));
		else
			n += wsprintf(pr + n, "(%i-BYTE) %s", dsize, (ideal == 0 ? "PTR " : ""));
		;
	};
	if ((putdefseg != 0 || seg != defseg) && seg != SEG_UNDEF)
		n += wsprintf(pr + n, "%s:", segname[seg]);
	if (ideal == 0) pr[n++] = '[';
	n += wsprintf(pr + n, "%s", descr);
	...
	if (mode >= DISASM_DATA) {
		...
		case MRJ:                        // Memory/reg in ModRM as JUMP target
			...
			if (mode >= DISASM_FILE && shownear != 0)
				nresult += wsprintf(da->result + nresult, "%s ", (lowercase ? "near" : "NEAR"));
			break;
		...
		case MMS:                        // Memory in ModRM byte (as SEG:OFFS)
			...
			if (datasize == 4 && inmemory)
				da->memtype = DEC_FWORD;
			if (mode >= DISASM_FILE)
				nresult += wsprintf(da->result + nresult, "%s ", (lowercase ? "far" : "FAR"));
			break;
...
static void DecodeIM(int constsize, int sxt, int type) {
	...
	if (mode >= DISASM_FILE && da->error == DAE_NOERR) {
		...
		if (i != 0 && symbolic != 0) {
			...
			nresult += wsprintf(da->result + nresult, "%lX", data);
		else
			nresult += wsprintf(da->result + nresult, "-%lX", -data);
...
static void DecodeC1(void) {
	if (mode < DISASM_DATA) return;
	da->immconst = 1;
	if (mode >= DISASM_FILE) nresult += wsprintf(da->result + nresult, "1");
};
...
static void DecodeRJ(ulong offsize, ulong nextip) {
	...
	if (addr == 0) da->zeroconst = 1;
	if (mode >= DISASM_FILE) {
		if (offsize == 1) nresult += wsprintf(da->result + nresult,
		...
		if (mode >= DISASM_CODE)
			...
			i = 0;
		if (symbolic == 0 || i == 0)
			nresult += wsprintf(da->result + nresult, "%08lX", addr);
		else
			nresult += wsprintf(da->result + nresult, "%.*s", TEXTLEN - nresult - 25, s);
...
static void DecodeJF(void) {
	...
	if (mode >= DISASM_FILE) {
		nresult += wsprintf(da->result + nresult, "%s %04X:%08X",
...
static void DecodeSG(int index) {
	...
	if (mode >= DISASM_FILE) {
		i = wsprintf(da->result + nresult, "%s", segname[index]);
...
static void DecodeCR(int index) {
	...
	if (mode >= DISASM_FILE) {
		index = (index >> 3) & 0x07;
		nresult += wsprintf(da->result + nresult, "%s", crname[index]);
		if (lowercase) _strlwr(da->result + nresult);
	};
};
...
static void DecodeDR(int index) {
	...
	if (mode >= DISASM_FILE) {
		index = (index >> 3) & 0x07;
		i = wsprintf(da->result + nresult, "%s", drname[index]);
		if (lowercase) _strlwr(da->result + nresult);
		nresult += i;
	};
};
...
ulong Disasm(char *src, ulong srcsize, ulong srcip, t_disasm *disasm, int disasmmode)
{
	...
	while (size > 0) {
		...
		if (mode >= DISASM_FILE)
			ndump += wsprintf(da->dump + ndump, "%02X:", *cmd);
		da->nprefix++;
		cmd++; srcip++; size--; u++;
	};
	...
	if (repeated) {
		if (mode >= DISASM_FILE) {
			...
			nresult += wsprintf(da->result + nresult, "PREFIX %s:", pname);
	...
	if (lockprefix != 0) {
		if (mode >= DISASM_FILE) nresult += wsprintf(da->result + nresult, "LOCK ");
	...
	if (da->error != 0) {                  // Hard error in command detected
		if (mode >= DISASM_FILE)
			nresult = wsprintf(da->result, "???");
		if (da->error == DAE_BADCMD &&
			(*cmd == 0x0F || *cmd == 0xFF) && size > 0
			) {
			if (mode >= DISASM_FILE) ndump += wsprintf(da->dump + ndump, "%02X", *cmd);
			cmd++; size--;
		};
		if (size > 0) {
			if (mode >= DISASM_FILE) ndump += wsprintf(da->dump + ndump, "%02X", *cmd);
		...
		if (mode >= DISASM_FILE) {
			ndump += wsprintf(da->dump + ndump, "%02X", *cmd++);
			if (hasrm) ndump += wsprintf(da->dump + ndump, "%02X", *cmd++);
			if (hassib) ndump += wsprintf(da->dump + ndump, "%02X", *cmd++);
			if (dispsize != 0) {
				da->dump[ndump++] = ' ';
