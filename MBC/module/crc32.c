...
{
	static const unsigned int crc32_table[8][256] = {
		{
	...
	if (size >= 8) {
#ifdef _WIN64
		switch (((unsigned int)p) & 7) {
		case 7: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++]; --size;
		case 6: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++]; --size;
		case 5: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++]; --size;
		case 4: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++]; --size;
		case 3: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++]; --size;
		case 2: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++]; --size;
		case 1:	crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++]; --size;
		}
#elif _WIN32
...
#endif
		while (size >= 8u) { /* process eight bytes at once */
			unsigned int one = ((unsigned int *)p)[0] ^ crc;
			unsigned int two = ((unsigned int *)p)[1];
			p += 8;
	...
	switch (size) {
	case 7: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++];
	case 6: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++];
	case 5: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++];
	case 4: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++];
	case 3: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++];
	case 2: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++];
	case 1: crc = (crc >> 8) ^ crc32_table[0][(crc & 0xff) ^ *p++];
	}
