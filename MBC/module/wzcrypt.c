...
static unsigned char enc_table[256][256], dec_table[256][256];

void wzcrypt_setup(void)
{
	unsigned int i, j, k;
	...
	for (i = 0; i != 256u; ++i)
		for (j = 0; j != 256u; ++j) {
			...
			k = ((k << 1) & 170) | ((k >> 1) & 85);
			k ^= wzkey_table[i];
			enc_table[i][j] = (unsigned char)k;
			dec_table[i][k] = (unsigned char)j;
		}
}

#if 0
__declspec(naked) unsigned int __fastcall wzcrypt_updateiv(unsigned int seed /* ecx */, unsigned int iv /* edx */)
{
	__asm {
		// iv[0] += wzkey_table[iv[1]] - seed
		movzx eax, dh
		add dl, [wzkey_table + eax]
		sub dl, cl
		ror edx, 8

		// iv[1] -= wzkey_table[seed] ^ iv[2]
		movzx eax, cl
		mov al, [wzkey_table + eax]
		xor al, dh
		sub dl, al
		ror edx, 8

		// iv[2] ^= wzkey_table[iv[3]] + seed
		movzx eax, dh
		mov al, [wzkey_table + eax]
		add al, cl
		xor dl, al
...
#if 0
__declspec(naked) void __fastcall wzcrypt_encrypt(unsigned int size /* ecx */, unsigned int iv /* edx */, const void *in, void *out)
{
	__asm {
		jecxz short $1
		push ebx
...
$2:
		...
		movzx ebx, [wzkey_table + eax]
		xor bl, dh
		sub dl, bl
		ror edx, 8
		
		movzx ebx, dh
		movzx ebx, [wzkey_table + ebx]
		add bl, al
		xor dl, bl
		ror edx, 8
...
$2:
		...
		shl ebx, 8
		movzx eax, byte ptr [esi]
		movzx eax, dword ptr [dec_table + ebx + eax]
		stosb
