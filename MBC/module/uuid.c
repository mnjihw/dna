...
int uuid_check_smbios(void)
{
	...
	VIRTUALIZER2_START
	{
		SECURE_API(s_GetSystemFirmwareTable)('RSMB', 0, data, sizeof(data));
		*(int *)data = 0;
...
void uuid_diskinfo(sha1_t *sha1)
{
	HANDLE handle;
	DWORD bytes;
	char data[1024];
	...
	VIRTUALIZER2_START
	{
		if (SECURE_API(s_GetVolumeInformationA)((*(int *)data = '\\:C', data), NULL, 0, (DWORD *)data, NULL, NULL, NULL, 0))
			sha1_update(sha1, (*(unsigned int *)&data[4] = crcsum32(*(unsigned int *)data, data, 4u), data), 8);

		if ((handle = (HANDLE)SECURE_API(s_CreateFileA)((*(int *)&data[0] = '\\\\.\\', *(int *)&data[4] = 'syhP', *(int *)&data[8] = 'laci', *(int *)&data[12] = 'virD', *(int *)&data[16] = '0e', data), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL)) != INVALID_HANDLE_VALUE) {
			if (SECURE_API(s_DeviceIoControl)(handle, SMART_GET_VERSION, NULL, 0, data, sizeof(GETVERSIONINPARAMS), &bytes, NULL) && bytes == sizeof(GETVERSIONINPARAMS)) {
				memset(data, 0, sizeof(SENDCMDINPARAMS) - 1);
				((SENDCMDINPARAMS *)data)->cBufferSize = 512;
				((SENDCMDINPARAMS *)data)->irDriveRegs.bCommandReg = IDE_ATA_IDENTIFY;
				if (SECURE_API(s_DeviceIoControl)(handle, SMART_RCV_DRIVE_DATA, data, sizeof(SENDCMDINPARAMS) - 1, &data[sizeof(SENDCMDINPARAMS) - 1], sizeof(SENDCMDOUTPARAMS) - 1 + 512, &bytes, NULL)) {
					IDSECTOR *sec = (IDSECTOR *)&data[sizeof(SENDCMDINPARAMS) - 1 + sizeof(SENDCMDOUTPARAMS) - 1];
					sha1_update(sha1, sec->sSerialNumber, sizeof(sec->sSerialNumber));
					sha1_update(sha1, sec->sModelNumber, sizeof(sec->sModelNumber));
				}
			}
			SECURE_API(s_CloseHandle)(handle);
		}
	}
	VIRTUALIZER_END
}

__declspec(naked) void __cdecl _cpuid(void *data, unsigned int code)
{
	__asm {
		VIRTUALIZER2_START
		{
		...
		*buffer = '\0';
		if (uuid_check_smbios() == 3) {
			...
			if (crcsum32(0xBADF00Du, &sha1, sizeof(sha1)) == 0x33999385u) {
				...
				sha1_update(&sha1, &sha1, sizeof(sha1));
				sha1_final(&sha1);
				if (crcsum32(0xBADF00Du, &sha1, sizeof(sha1)) == 0xCC3C7D00u) {
					sha1_init(&sha1);
					...
					sha1_update(&sha1, data, 16);
					SECURE_API(s_GlobalMemoryStatusEx)((memset(data, 0, sizeof(MEMORYSTATUSEX)), ((MEMORYSTATUSEX *)data)->dwLength = sizeof(MEMORYSTATUSEX), (MEMORYSTATUSEX *)data));
					sha1_update(&sha1, &((MEMORYSTATUSEX *)data)->ullTotalPhys, 8u);
					SECURE_API(s_GetNativeSystemInfo)((memset(data, 0, sizeof(SYSTEM_INFO)), (SYSTEM_INFO *)data));
