...
#define USERINFO 0x59
#define SERVERINFO 0x14F
#define CHARACTER_INFO 0x192
#define CHATTING 0x194

#define S_CHATTING 0xB5
...
unsigned int adapter_sel, target_id, operating, promiscuous_mode, currentChannel, breakloop;
ADAPTER *npf_adapter;
...
void packet_handle(session_t *s, unsigned char *data, unsigned int size)
{
	...
	case SERVERINFO: //serverinfo
		...
		if (currentChannel == 20)
			wsprintf(buf, "현재 채널 : %u세", currentChannel);
		else
			wsprintf(buf, "현재 채널 : %u채널", currentChannel);
	...
	case CHARACTER_INFO: //캐릭터 정보
		...
		break;
	case CHARACTER_INFO + 1: //캐릭터 사라질 때
		for (i = 0; i < _countof(character_list); ++i)
		{
			if (character_list[i].id == *(unsigned int*) &data[2])
			{
	...
	case CHATTING: //모두에게 채팅 op코드
		...
		if (FindWindow("#32770", "foo"))
		{
			...
			*(unsigned int*) &packet.data[2] = GetTickCount();
			j = *(unsigned short*) &data[7];
			*(unsigned short*) &packet.data[6] = j;
			memcpy(&packet.data[8], &data[9], j);
			*(unsigned char*)&packet.data[8 + j + 1] = 0;
			packet.size = 8 + j + 1;
