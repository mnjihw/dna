...
char* gen_uuid(char buffer[48])
{
	...
	*buffer = '\0';
	if (uuid_check_smbios() == 3)
	{
		...
		sha1_init(&sha1);
		cpuid((memset(&data, 0, 16), data), 0);
		sha1_update(&sha1, data, 16);
		cpuid((memset(&data, 0, 16), data), 0x80000002u);
		sha1_update(&sha1, data, 16);
		cpuid((memset(&data, 0, 16), data), 0x80000003u);
		sha1_update(&sha1, data, 16);
		cpuid((memset(&data, 0, 16), data), 0x80000004u);
		sha1_update(&sha1, data, 16);
